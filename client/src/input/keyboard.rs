...
pub fn is_action_just_pressed(action: GameAction, keyboard_input: &ButtonInput<KeyCode>, key_map: &KeyMap) -> bool {
    if let Some(key_codes) = key_map.map.get(&action) {
        for key_code in key_codes {
...
pub fn is_action_just_released(action: GameAction, keyboard_input: &ButtonInput<KeyCode>, key_map: &KeyMap) -> bool {
    if let Some(key_codes) = key_map.map.get(&action) {
        for key_code in key_codes {
