...
pub fn spawn_camera(mut commands: Commands) {
    commands
        ...
        .insert(CameraController::default()) // adds the CameraController
        .insert({
            let mut raycast_source = RaycastSource::<BlockRaycastSet>::default(); // default initialization
            raycast_source.cast_method = RaycastMethod::Transform; // use the camera's transform for raycasting
            raycast_source // return the object
        });
}
        ...
        for (mut camera_transform, mut controller) in camera_query.iter_mut() {
            ...
            if player.view_mode == ViewMode::FirstPerson {
                ...
                camera_transform.translation = Vec3::new(
                    player_position.x,
                    player_position.y + 1.8, // adjust height for the player's eyes
                    player_position.z,
                );
